...
import (
	...
	omni "github.com/voxgig-sdk/bank-logos-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.BankLogosError); is {
				out := map[string]any{
					"name":    "BankLogosError",
...
func omniCtx(arg any, client *sdk.BankLogosSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.BankLogosSDK); is {
			client = live

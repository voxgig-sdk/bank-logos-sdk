# BankLogos Golang SDK Reference

Complete API reference for the BankLogos Golang SDK.
...
#### `TestSDK(testopts, sdkopts map[string]any) *BankLogosSDK`
...
#### `Logo(data map[string]any) BankLogosEntity`
...
```go
result, err := client.Logo(nil).Load(map[string]any{"bank": "bank"}, nil)
if err != nil {
    panic(err)
}
fmt.Println(result)

...
The Lua SDK for the BankLogos API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:Logo()` — each with the same small set of operations (`load`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
```lua
local logo, err = client:Logo():load({ bank = "example" })
if err then error(err) end
```
...
```lua
local logo, err = client:Logo():load({ bank = "bank" })
```
...
```
lua/
├── bank-logos_sdk.lua    -- Main SDK module
...
The main module (`bank-logos_sdk`) exports the SDK constructor

...
[MCP](https://modelcontextprotocol.io) server exposing the BankLogos SDK as
two agent tools — `bank-logos_list` and `bank-logos_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/bank-logos-mcp)
make build

# 2. Provide credentials via the environment
export BANK_LOGOS_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user bank-logos \
  -- /absolute/path/to/bank-logos-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./bank-logos-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// bank-logos_list: first page of records
{ "entity": "logo" }
{ "entity": "logo", "query": { } }

// bank-logos_load: one record by id
{ "entity": "logo", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/bank-logos-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user bank-logos \
     -- "$PWD"/dist/*/bank-logos-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `bank-logos_list` and `bank-logos_load` tools now appear
   in new sessions. Ask the agent to *"list logo using bank-logos"*
   and it calls `bank-logos_list` with `{"entity":"logo"}`.
...
```sh
export BANK_LOGOS_APIKEY=sk_live_xxx            # API key
export BANK_LOGOS_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `bank-logos_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `bank-logos_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `BANK_LOGOS_APIKEY` | API key sent with every request. |
| `BANK_LOGOS_BASE` | Optional override of the API base URL. |
...
```sh
./bank-logos-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"bank-logos_load","arguments":{"entity":"logo","query":{"id":1}}}}'
```

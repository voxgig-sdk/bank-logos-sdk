...
MCP server exposing the BankLogos SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `bank-logos_list` | `entity`, optional `query` map | First page of records as JSON |
| `bank-logos_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `bank-logos_list` and
`bank-logos_load` tools become available in new sessions.

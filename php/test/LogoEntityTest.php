...
class LogoEntityTest extends TestCase
{
    ...
    {
        $testsdk = BankLogosSDK::test(null, null);
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set BANK_LOGOS_TEST_LOGO_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function logo_basic_setup($extra)
{
    ...
    $client = BankLogosSDK::test($options, $extra);
    ...
    $entid_env_raw = getenv("BANK_LOGOS_TEST_LOGO_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["BANK_LOGOS_TEST_LOGO_ENTID"]);
    if ($idmap_resolved === null) {
        $idmap_resolved = Helpers::to_map($idmap);
    }

    if ($env["BANK_LOGOS_TEST_LIVE"] === "TRUE") {
        ...
        $client = new BankLogosSDK(Helpers::to_map($merged_opts));
    }
    ...
    return [
        ...
        "env" => $env,
        "explain" => $env["BANK_LOGOS_TEST_EXPLAIN"] === "TRUE",
        "live" => $live,

...
class BankLogosNetsimFeature extends BankLogosBaseFeature
{
    ...
    private function _simulate(BankLogosContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(BankLogosContext $ctx, array $applied): void
    {
        $client = $this->client;

...
module BankLogosUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = BankLogosResponse.new({})
    ...
    if fetch_err
      ...
      response = BankLogosResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = BankLogosResponse.new(fetched)
    else

...
class FtCtrl extends BankLogosControl
{
...
class FtClient
{
    ...
    private BankLogosUtility $utility;

    public function __construct(BankLogosUtility $utility, array $options)
    {
    ...
    public function get_utility(): BankLogosUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (BankLogosContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public BankLogosContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (BankLogosContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = BankLogosConfig::make_feature($name);
    ...
    private static function buildUrl(BankLogosSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof BankLogosError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new BankLogosResponse($fetched);
        }
    ...
    private function fail(BankLogosContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (BankLogosContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;

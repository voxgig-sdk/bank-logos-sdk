...
boru-driven command-line client **and** interactive REPL for the BankLogos
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/bank-logos-cli)
make build
...
./bank-logos-cli load 1 logo            # {id:1} shorthand
./bank-logos-cli load '{id:1}' logo       # explicit match map

# 5. Override the API base URL for a single call
BANK_LOGOS_BASE=https://api.example.com ./bank-logos-cli load 1 logo

# 6. No arguments -> interactive REPL
./bank-logos-cli
bank-logos> load 1 logo
bank-logos> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/bank-logos-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/bank-logos-cli load 1 logo
   ```
...
```sh
./bank-logos-cli load 1 logo          # scalar shorthand for {id:1}
./bank-logos-cli load '{id:1}' logo     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `bank-logos>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `logo`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/bank-logos-cli`. |
...
(`./bank-logos-cli <expr>`) and interactively (the REPL), and expressions compose the

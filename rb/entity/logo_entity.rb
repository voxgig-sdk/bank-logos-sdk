...
class LogoEntity
  ...
  def data_set(args)
    if args
      @_data = BankLogosHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = BankLogosHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Logo, Hash] the loaded Logo; raises BankLogosError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = BankLogosHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end

...
require_relative 'BankLogos_types'
...
class BankLogosSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = BankLogosHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = BankLogosHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, BankLogosFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    BankLogosUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = BankLogosHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = BankLogosHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = BankLogosHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => BankLogosError.new(
        "#{op}_allow",
        "BankLogosSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = BankLogosHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = BankLogosError.new(
        "graphql_error", "BankLogosSDK: graphql: #{msg}")
      res["graphql"] = errors
    end

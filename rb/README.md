...
The Ruby SDK for the BankLogos API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.Logo` — with named operations (`load`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
```ruby
client = BankLogosSDK.test(testopts, sdkopts)
```
...
raise a `BankLogosError` (a `StandardError` subclass), so wrap
...
```
rb/
├── BankLogos_sdk.rb       -- Main SDK module
...
The main module (`BankLogos_sdk`) exports the SDK class

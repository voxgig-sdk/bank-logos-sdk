...
//     (\`new BankLogosSDK(...)\` / \`BankLogosSDK.test(...)\` become
//     \`BankLogosSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"logo":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new BankLogosSDK(...)\` and any
// \`BankLogosSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+BankLogosSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/BankLogosSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)

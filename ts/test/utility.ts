...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.BANKLOGOS_TEST_LIVE ||
    'TRUE' === process.env.BANKLOGOS_TEST_OVERRIDE
  ) {
  ...
  m.BANKLOGOS_TEST_EXPLAIN = process.env.BANKLOGOS_TEST_EXPLAIN || m.BANKLOGOS_TEST_EXPLAIN

# BankLogos TypeScript SDK



The TypeScript SDK for the BankLogos API — a type-safe, entity-oriented client with full async/await support.

The API is exposed as capitalised, semantic **Entities** — e.g.
`client.Logo()` — each with a small set of operations (`load`)
...
```ts
const client = new BankLogosSDK({ apikey: '...' })
...
await entity.load({ bank: 'example_bank' })
...
| `tester(testopts?, sdkopts?)` | `BankLogosSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `BankLogosSDK.test(testopts?, sdkopts?)` | `BankLogosSDK` | Create a test-mode client. |
...
| `client` | `client(): BankLogosSDK` | Return the parent SDK client. |
...
```ts
const logo = await client.Logo().load({ bank: 'bank' })
```
...
bank-logos/
├── src/
│   ├── BankLogosSDK.ts        # Main SDK class

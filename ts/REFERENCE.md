# BankLogos TypeScript SDK Reference

Complete API reference for the BankLogos TypeScript SDK.
...
```ts
new BankLogosSDK(options?: object)
```
...
#### `BankLogosSDK.test(testopts?, sdkopts?)`
...
**Returns:** `BankLogosSDK` instance in test mode.
...
```ts
const result = await client.Logo().load({ bank: 'bank' })
```
...
Return the parent `BankLogosSDK` instance.
